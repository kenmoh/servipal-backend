...
async def test_get_wallet_details(mock_supabase):
    ...
    await mock_supabase.table("wallets").insert({
        "user_id": str(user_id),
        "balance": 5000.00,
...
async def test_initiate_wallet_top_up(mock_supabase):
    ...
    with pytest.MonkeyPatch.context() as m:
        m.setattr("app.services.wallet_service.save_pending", 
                  lambda key, value, expire=None: None) # Mock sync/async? 
        ...
        # We should patch it.
        async def mock_get_contact(): return {"email": "test@test.com", "phone": "123"}
        m.setattr("app.services.wallet_service.get_customer_contact_info", mock_get_contact)
        
        # Mock settings.FLUTTERWAVE_PUBLIC_KEY
        m.setattr("app.services.wallet_service.settings.FLUTTERWAVE_PUBLIC_KEY", "FLWPUBK-TEST")
