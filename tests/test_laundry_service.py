...
async def test_initiate_laundry_payment(mock_supabase):
    ...
    # Mock Service Item
    await mock_supabase.table("laundry_services").insert({
        "id": str(item_id),
        "name": "Wash",
        "price": 2000,
    ...
    with pytest.MonkeyPatch.context() as m:
        async def mock_save(*args, **kwargs): return "pending_456"
        m.setattr("app.services.laundry_service.save_pending", mock_save)
        
        result = await initiate_laundry_payment(
            data, user_id, {"email": "test@example.com", "phone": "123456"}, mock_supabase
        )
