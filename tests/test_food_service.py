...
async def test_initiate_food_payment(mock_supabase):
    ...
    # Mock Charges
    mock_supabase._data["charges_and_commissions"] = [{"base_delivery_fee": 500, "delivery_fee_per_km": 100}]
    
    # Mock Vendor Menu Item
    await mock_supabase.table("menu_items").insert({
        "id": str(item_id),
        "name": "Burger",
        "price": 1500,
    ...
    data = CheckoutRequest(
        vendor_id=vendor_id,
        items=[CartItem(item_id=item_id, name="Burger", price=Decimal("1500"), quantity=2)],
        delivery_option="PICKUP",
        cooking_instructions="No onions"
    )
    
    with pytest.MonkeyPatch.context() as m:
        async def mock_save(*args, **kwargs): return "pending_123"
