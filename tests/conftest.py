...
class MockRPCBuilder:
    ...
    async def execute(self):
        ...
        if self.name == "get_vendor_detail_with_menu":
             ...
             return MockResponse([
                 {
                     "vendor_json": {"id": vendor_id, "store_name": "Test Vendor", "phone_number": "+2348000000000"},
                     "category_json": {"id": str(uuid4()), "name": "Cat 1"},
                     "item_json": {"id": str(uuid4()), "name": "Item 1", "price": 1000, "in_stock": True}
                 }
             ])
        if self.name == "update_wallet_balance":
             ...
             wallet = next((w for w in wallets if str(w["user_id"]) == str(user_id)), None)
             if wallet:
...
class MockQueryBuilder:
    ...
    def _add_defaults(self, item):
        if self.table_name == "profiles":
            defaults = {
                "email": None, "phone_number": "+2348000000000", "full_name": None, "user_type": "CUSTOMER",
                "store_name": None, "business_name": None, "profile_image_url": None, "backdrop_image_url": None,
                "is_online": False, "is_verified": False, "is_blocked": False, "account_status": "ACTIVE",
                "last_seen_at": None, "created_at": datetime.now().isoformat(), "updated_at": datetime.now().isoformat(),
                "dispatcher_id": None, "bike_number": None, "business_address": None, "state": None,
                "can_pickup_and_dropoff": False, "pickup_and_delivery_charge": 0.0,
                "total_deliveries": 0, "average_rating": 0.0, "review_count": 0, "total_distance_travelled": 0.0,
                "has_delivery": False, "order_cancel_count": 0, "rider_is_suspended_for_order_cancel": False
            }
        elif self.table_name == "disputes":
            defaults = {
                "resolution_notes": None, "resolved_by_id": None, "resolved_at": None,
                "created_at": datetime.now().isoformat(), "updated_at": datetime.now().isoformat()
            }
        else:
            defaults = {"created_at": datetime.now().isoformat(), "updated_at": datetime.now().isoformat()}
    ...
    async def execute(self):
        ...
        if hasattr(self, 'operation'):
            if self.operation == "insert":
                payloads = self.data_payload if isinstance(self.data_payload, list) else [self.data_payload]
                to_return = []
                for item in payloads:
                    new_item = item.copy()
                    if "id" not in new_item: new_item["id"] = str(uuid4())
                ...
                for p in payloads:
                    ...
                    idx = None
                    if "id" in new_item:
                        idx = next((i for i, d in enumerate(table_data) if str(d["id"]) == str(new_item["id"])), None)
                ...
                self.db[self.table_name] = [row for row in table_data if row.get("id") not in candidate_ids]
        ...
        if self.is_single:
            if not results: return MockResponse(None)
    ...
    def _apply_filters(self, data):
        ...
        for col, op, val in self.query_filters:
            if op == "eq": filtered = [r for r in filtered if str(r.get(col)) == str(val)]
            elif op == "neq": filtered = [r for r in filtered if str(r.get(col)) != str(val)]
            elif op == "lt": filtered = [r for r in filtered if r.get(col) is not None and r.get(col) < val]
            elif op == "gt": filtered = [r for r in filtered if r.get(col) is not None and r.get(col) > val]
            elif op == "gte": filtered = [r for r in filtered if r.get(col) is not None and str(r.get(col)) >= str(val)]
            elif op == "lte": filtered = [r for r in filtered if r.get(col) is not None and str(r.get(col)) <= str(val)]
            elif op == "in": filtered = [r for r in filtered if str(r.get(col)) in [str(v) for v in val]]
            elif op == "or":
                if "," in val:
                    ...
                    for c in conditions:
                        ...
                        if len(parts) == 3 and parts[1] == "eq":
                            c_col, _, c_val = parts
                            or_results.extend([r for r in data if str(r.get(c_col)) == str(c_val)])
                    seen = set()
                    unique_res = []
                    for r in or_results:
                        if id(r) not in seen:
                            seen.add(id(r)); unique_res.append(r)
                    filtered = unique_res
        return filtered
...
class MockSupabaseAuth:
    ...
    async def sign_up(self, credentials):
        ...
        if email not in self.users:
            ...
            profile_data = {
                "id": user_id, "email": email, "full_name": metadata.get("full_name"),
                "phone_number": metadata.get("phone", "+2348000000000"), "user_type": metadata.get("user_type", "CUSTOMER")
            }
    ...
    @property
    def admin(self): return self.Admin(self)

class MockSupabaseClient:
    def __init__(self):
        self._data = {
            "disputes": [], "dispute_messages": [], "profiles": [], "users": [], 
            "wallets": [], "transactions": [], "product_items": [], "product_orders": [],
            "delivery_orders": [], "reviews": [], "food_vendors": [], "laundry_vendors": [],
            "charges_and_commissions": [{"base_delivery_fee": 500, "delivery_fee_per_km": 100}],
            "profile_images": [], "food_orders": [], "laundry_orders": [], "menu_items": [], "laundry_services": [],
            "fcm_tokens": [], "audit_logs": []
        }
        self.auth = MockSupabaseAuth(self._data)
    def table(self, name): return MockQueryBuilder(name, self._data)
    def rpc(self, name, params=None): return MockRPCBuilder(self._data, name, params or {})

@pytest.fixture
def mock_supabase(): return MockSupabaseClient()
