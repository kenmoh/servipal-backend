...
async def test_get_my_disputes(mock_supabase):
    ...
    await mock_supabase.table("disputes").insert({
        "id": str(uuid4()),
        "order_id": str(uuid4()),
        "order_type": "FOOD",
...
async def test_get_dispute_detail(mock_supabase):
    ...
    # Add a message
    await mock_supabase.table("dispute_messages").insert({
        "id": str(uuid4()),
...
async def test_resolve_dispute(mock_supabase):
    ...
    # Setup transaction
    await mock_supabase.table("transactions").insert({
        "id": str(uuid4()),
        "order_id": str(order_id),
    ...
    resolve_data = DisputeResolve(
        ...
        notes="Refund processed for valid reason"
    )
    ...
    with pytest.MonkeyPatch.context() as m:
        ...
        result = await resolve_dispute(dispute_id, resolve_data, admin_id, mock_supabase)
    
    assert result["success"] is True
    
    # Verify DB status update
    dispute = (await mock_supabase.table("disputes").select("status").eq("id", str(dispute_id)).single().execute()).data
