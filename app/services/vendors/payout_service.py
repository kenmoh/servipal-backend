...
class TransferService:
    ...
    async def create_transfer(
        ...
        payout_response_data = PayoutData(
            ...
            account_number=str(response_data.get("account_number", "")),
            bank_code=str(response_data.get("bank_code", "")),
            full_name=str(response_data.get("full_name", "")),
            created_at=response_data.get("created_at"),
            currency=str(response_data.get("currency", "NGN")),
            debit_currency=str(response_data.get("debit_currency", "NGN")),
            amount=Decimal(str(response_data.get("amount", 0))),
            fee=Decimal(str(response_data.get("fee", 0))),
            status=str(response_data.get("status", "NEW")),
            reference=str(response_data.get("reference", "")),
            meta=str(response_data.get("meta", "")) if response_data.get("meta") is not None else None,
            narration=str(response_data.get("narration", "")),
            complete_message=str(response_data.get("complete_message", "")),
            requires_approval=int(response_data.get("requires_approval", 0)),
            is_approved=int(response_data.get("is_approved", 0)),
            bank_name=str(response_data.get("bank_name", "")),
        )
        await (
