...
async def list_laundry_orders(
    ...
    return LaundryOrderListResponse(
        data=[LaundryOrderSummary(**r) for r in rows],
        meta=PaginationMeta(
            total=total,
            page=page,
            page_size=page_size,
...
async def get_laundry_order(
    ...
    result = await supabase.rpc(
        "admin_get_laundry_order_detail",
    ...
    if not result.data:
        raise HTTPException(
            ...
            detail=f"Laundry order {order_id} not found",
        )
    ...
    items = [LaundryOrderItem(**i) for i in (row.get("items") or [])]
